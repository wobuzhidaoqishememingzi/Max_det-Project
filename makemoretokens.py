...
class ModelConfig:
    ...
    n_embd2: int = 64 # embedding dimension for another MLP layer
    n_head: int = 4 # number of attention heads in a transformer
...
class CausalSelfAttention(nn.Module):
    ...
    """
    """causal means that the attention is only applied to the left(previous) tokens, not the right(future) tokens.
    this whole class is equivlent to the nn.MultiheadAttention module, but with a causal mask applied.
    Self attention is the core of the transformer architecture, it allows the model to focus on different parts of the input sequence,
    and learn relationships between tokens (see which tokens is more important) regardless of their position in the sequence.
    (会根据上下文智能选重点)
    """
    ...
    def forward(self, x):
        ...
        # ‘Split out’ the attention heads, with each head focusing on a portion of the information.
        q, k ,v  = self.c_attn(x).split(self.n_embd, dim=2)
        k = k.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        q = q.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        v = v.view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)

        # causal self-attention (score); Self-attend: (B, nh, T, hs) x (B, nh, hs, T) -> (B, nh, T, T)
        att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
        """Using a mask, blocked out future information (屏蔽). 
            The masked position becomes -inf and 0 after softmax."""
        att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float('-inf')) 
        """softmax: sum up to 1 and normalize the attention scores.
        the higher the score, the more attention(prob) is paid to that token."""
        att = F.softmax(att, dim=-1)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
        y = y.transpose(1, 2).contiguous().view(B, T, C) # re-assemble all head outputs side by side

        # output projection, the new vector after self-attention, this can send to the next layer
        y = self.c_proj(y)
        return y
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        """use the previous class to make it only give attention to the left"""
        self.attn = CausalSelfAttention(config)
        """do the normalization and then send to the MLP(Multilayer Perceptron) layer"""
        self.ln_2 = nn.LayerNorm(config.n_embd)
        """两层的MLP: c_fc: 把维度放大 4 倍（中间层）
            act: 用的是 GELU 激活函数
            c_proj: 再投影回原来的维度（输出层）"""
        ...
        m = self.mlp
        """input x, then use c_fc to project it to a larger dimension, then use GELU activation,
        then use c_proj to project it back to the original dimension."""
        self.mlpf = lambda x: m.c_proj(m.act(m.c_fc(x))) # MLP forward

    def forward(self, x):
        x = x + self.attn(self.ln_1(x)) # add the self attention output to the input
        x = x + self.mlpf(self.ln_2(x)) # add the MLP output to the input
        return x
...
class Transformer(nn.Module):
    ...
    def __init__(self, config):
        ...
        """
        wte: word token embedding table, each token turn into a vector of size n_embd;
        wpe: word position embedding table, each position turn into a vector of size n_embd;
        h: hidden layers; 
        ln_f: final layer normalization, to normalize the output of the last hidden layer.
        """
    ...
    def forward(self, idx, targets=None):
        ...
        for block in self.transformer.h:
            """passes through each Transformer Block layer in turn (each layer contains both Attention and MLP)."""
            x = block(x)
        x = self.transformer.ln_f(x)
        """normalize and then each position outputs a vector of length vocab_size representing the probability of each word."""
        logits = self.lm_head(x)

        # if we are given some desired targets also calculate the loss
        loss = None
        if targets is not None:
            """use cross entropy loss to compare the logits with the targets."""
...
"""BoW: doesn't care about the order of tokens, only consider which words are present in the input sequence.
CausalBoW: care about the order, have positional embedding, 
only take the previous block_size tokens, encode them with a lookup table, predict the next token
"""
...
class CausalBoW(nn.Module):
    ...
    def __init__(self, config):
        ...
        """bias is a lower triangular matrix of ones, which is used to mask out the future tokens 
        (only see itself and the previous one)."""
        self.block_size = config.block_size
        self.register_buffer("bias", torch.tril(torch.ones(config.block_size, config.block_size))
                            .view(1, config.block_size, config.block_size))

    def forward(self, x):
        """ same logic as in CausalSelfAttention, but we do not have any query, key, value projections"""
...
class BoWBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        m = self.mlp
        """input x, then use c_fc to linear project it to a larger dimension, 
        then use tanh activation(not like transformer using GELU), 
        then use c_proj to project it back to the original dimension."""
        self.mlpf = lambda x: m.c_proj(F.tanh(m.c_fc(x))) # MLP forward

    def forward(self, x):
        """residual connections: original x + BoW features + MLP features;
        keep the original x to make the training easier 
        (even cbow and mlpf learn nothing, the modle still have info of x itself).
        """
        x = x + self.cbow(x)
        x = x + self.mlpf(x)
        return x
...
"""
"""read the token sequence one by one(in order), update the hidden state(记忆状态) at each step
"""
...
class GRUCell(nn.Module):
    ...
    each step calculates a reset gate r, an update gate z and a candidate
    hidden state hbar.
    this can help the model to learn long-term dependencies better 
    (decide which info should update, forget, or keep)
    """
    def __init__(self, config):
        super().__init__()
